...
public class VideoTexture_FullScreen : MonoBehaviour 
{
	...
	{
		index = Mathf.Lerp(firstFrame,lastFrame,f   );
		Sync();
	}

	public void syncToPosition(float targetPos){

		if(Mathf.Abs(currentPosition-targetPos)>0.01 ){
	...
	public void loadMovie(string name){
		...
		if(enableAudio){
			AudioSource clipAudio = this.GetComponent<AudioSource>();
			AudioClip thisClip = Resources.Load(name) as AudioClip;
