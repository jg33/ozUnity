Shader "Hidden/CC_Blend"
{
	...
	// Darken
	fixed4 frag_darken(v2f_img i):COLOR
	{
	...
	// Multiply
	fixed4 frag_multiply(v2f_img i):COLOR
	{
		half4 o = tex2D(_OverlayTex, i.uv);
		half4 c = tex2D(_MainTex, i.uv);
	...
	// Color Burn
	fixed4 frag_colorburn(v2f_img i):COLOR
	{
		half4 o = tex2D(_OverlayTex, i.uv);
		half4 c = tex2D(_MainTex, i.uv);
	...
	// Linear Burn
	fixed4 frag_linearburn(v2f_img i):COLOR
	{
		half4 o = tex2D(_OverlayTex, i.uv);
		half4 c = tex2D(_MainTex, i.uv);
	...
	// Darker Color
	fixed4 frag_darkercolor(v2f_img i):COLOR
	{
		half4 o = tex2D(_OverlayTex, i.uv);
		half4 c = tex2D(_MainTex, i.uv);
	...
	// Lighten
	fixed4 frag_lighten(v2f_img i):COLOR
	{
		half4 o = tex2D(_OverlayTex, i.uv);
		half4 c = tex2D(_MainTex, i.uv);
	...
	// Screen
	fixed4 frag_screen(v2f_img i):COLOR
	{
		half4 o = tex2D(_OverlayTex, i.uv);
		half4 c = tex2D(_MainTex, i.uv);
	...
	// Color Dodge
	fixed4 frag_colordodge(v2f_img i):COLOR
	{
		half4 o = tex2D(_OverlayTex, i.uv);
		half4 c = tex2D(_MainTex, i.uv);
		return lerp(c, c / (half4_one - o), _Amount); // Should check for div by 0 but GPU drivers don't care
	}

	// Linear Dodge (Add)
	fixed4 frag_add(v2f_img i):COLOR
	{
		half4 o = tex2D(_OverlayTex, i.uv);
		half4 c = tex2D(_MainTex, i.uv);
		return lerp(c, c + o, _Amount);
	...
	// Lighter Color
	fixed4 frag_lightercolor(v2f_img i):COLOR
	{
		half4 o = tex2D(_OverlayTex, i.uv);
		half4 c = tex2D(_MainTex, i.uv);
		half4 result = luminance(c.rgb) > luminance(o.rgb) ? c : o;
	...
	// Overlay
	fixed4 frag_overlay(v2f_img i):COLOR
	{
		half4 o = tex2D(_OverlayTex, i.uv);
		half4 c = tex2D(_MainTex, i.uv);
		half4 check = step(0.5, c);
		half4 result = check * (half4_one - ((half4_one - 2 * (c - 0.5)) * (half4_one - o))); 
		result += (half4_one - check) * (2 * c * o);
		return lerp(c, result, _Amount);
	}

	// Soft Light
	fixed4 frag_softlight(v2f_img i):COLOR
	{
		half4 o = tex2D(_OverlayTex, i.uv);
		half4 c = tex2D(_MainTex, i.uv);
		half4 check = step(0.5, o);
		half4 result = check * (2 * c * o + c * c - 2 * c * c * o); 
		result += (half4_one - check) * (2.0 * sqrt(c) * o - sqrt(c) + 2 * c - 2 * c * o);
		return lerp(c, result, _Amount);
	}

	// Hard Light
	fixed4 frag_hardlight(v2f_img i):COLOR
	{
		half4 o = tex2D(_OverlayTex, i.uv);
		half4 c = tex2D(_MainTex, i.uv);
		half4 check = step(0.5, o);
	...
	// Vivid Light
	fixed4 frag_vividlight(v2f_img i):COLOR
	{
		half4 o = tex2D(_OverlayTex, i.uv);
		half4 c = tex2D(_MainTex, i.uv);
		half4 check = step(0.5, o);
		half4 result = check * (c / (half4_one - 2 * (o - 0.5)));
		result += (half4_one - check) * (half4_one - (half4_one - c) / (2 * o));
		return lerp(c, result, _Amount);
	}

	// Linear Light
	fixed4 frag_linearlight(v2f_img i):COLOR
	{
		half4 o = tex2D(_OverlayTex, i.uv);
		half4 c = tex2D(_MainTex, i.uv);
		half4 check = step(0.5, o);
		half4 result = check * (c + (2 * (o - 0.5)));
		result += (half4_one - check) * (c + 2 * o - half4_one);
		return lerp(c, result, _Amount);
	}

	// Pin Light
	fixed4 frag_pinlight(v2f_img i):COLOR
	{
		half4 o = tex2D(_OverlayTex, i.uv);
		half4 c = tex2D(_MainTex, i.uv);
		half4 check = step(0.5, o);
		half4 result = check * max(2 * (o - 0.5), c);
		result += (half4_one - check) * min(2 * o, c);
		return lerp(c, result, _Amount);
	}

	// Hard Mix
	fixed4 frag_hardmix(v2f_img i):COLOR
	{
		half4 o = tex2D(_OverlayTex, i.uv);
		half4 c = tex2D(_MainTex, i.uv);
	...
	// Difference
	fixed4 frag_difference(v2f_img i):COLOR
	{
		half4 o = tex2D(_OverlayTex, i.uv);
		half4 c = tex2D(_MainTex, i.uv);
	...
	// Exclusion
	fixed4 frag_exclusion(v2f_img i):COLOR
	{
		half4 o = tex2D(_OverlayTex, i.uv);
		half4 c = tex2D(_MainTex, i.uv);
	...
	// Subtract
	fixed4 frag_subtract(v2f_img i):COLOR
	{
		half4 o = tex2D(_OverlayTex, i.uv);
		half4 c = tex2D(_MainTex, i.uv);
	...
	// Divide
	fixed4 frag_divide(v2f_img i):COLOR
	{
		half4 o = tex2D(_OverlayTex, i.uv);
		half4 c = tex2D(_MainTex, i.uv);
		return lerp(c, c / o, _Amount);
