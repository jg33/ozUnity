Shader "Hidden/CC_CrossStitch"
{
	...
	CGINCLUDE
		...
		fixed4 frag(v2f_img i):COLOR
		{
			...
			half4 color;
			color = (reminder.x == 0 || reminder.y == 0) ? tex2D(_MainTex, i.uv) * _Brightness : half4(0.0, 0.0, 0.0, 1.0);
			return color;
		}
		...
		fixed4 frag_invert(v2f_img i):COLOR
		{
			half2 reminder = stitch(i.uv);
			half4 color;
			color = (reminder.x == 0 || reminder.y == 0) ? half4(0.0, 0.0, 0.0, 1.0) : tex2D(_MainTex, i.uv) * _Brightness;
			return color;
		}
		...
		fixed4 frag_px(v2f_img i):COLOR
		{
			half2 reminder = stitch(i.uv);
			half4 color;
			color = (reminder.x == 0 || reminder.y == 0) ? pixelate(_MainTex, i.uv, _Scale, _Ratio) * _Brightness : half4(0.0, 0.0, 0.0, 1.0);
			return color;
		}
		...
		fixed4 frag_invert_px(v2f_img i):COLOR
		{
			half2 reminder = stitch(i.uv);
			half4 color;
			color = (reminder.x == 0 || reminder.y == 0) ? half4(0.0, 0.0, 0.0, 1.0) : pixelate(_MainTex, i.uv, _Scale, _Ratio) * _Brightness;
			return color;
		}
